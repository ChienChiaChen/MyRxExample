...
class AuthMessageQueueHandler private constructor() : Runnable {
    ...
    private val mMessageSubject: PublishSubject<AuthMessage> = PublishSubject.create<AuthMessage>()
    ...
    override fun run() {
        if (mCallMethodByUser) {
            throw IllegalAccessException("can't call run() method, use AuthMessageQueueHandler.getInstance() instead.")
        }
