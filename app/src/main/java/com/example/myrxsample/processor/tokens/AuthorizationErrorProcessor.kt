...
import com.example.myrxsample.error.login.NavigatorFragment
...
object AuthorizationErrorProcessor {
        ...
        return Observable.defer {
            synchronized(AuthorizationErrorProcessor::class.java) {
                return@synchronized when (mIsBlocking) {
                    true -> Observable.error<Boolean>(currentWaitLoginInQueue)
                    false -> {
                        this.updateIsBlockingState(true)
                        when (mLastRefreshTokenTimeStamp > lastRefreshStamp) {
                            true -> processLoginSuccessMessage(lastRefreshStamp)
                            false -> processLoginMessage(currentActivity, lastRefreshStamp)
                        }
                    }
                }
            }
    ...
    private fun processLoginMessage(
        ...
        return when (mLastCancelRefreshTokenTimeStamp > lastRefreshStamp) {
            true -> processLoginFailureMessage(lastRefreshStamp)
            false -> {
                NavigatorFragment
                    .startLoginForResult(currentActivity) //<--- 去 Login
                    .doOnSuccess {
                        when (it) {
                            true -> updateLoginSuccessTimeStamp()
                            false -> updateLoginFailureTimeStamp()
                        }
                    }
        ...
        return obs.flatMap { processorError ->
            when (processorError) {
                is AuthorizationErrorProcessResult.WaitLoginInQueue ->
                    AuthMessageQueueHandler.getInstance().sendMessage(processorError.lastRefreshStamp)
